...
summServer <- function(id, df1, df2, df3, df4){
 moduleServer(id, function(input, output, session){
  ...
  anom <- reactive({
   return(
    getSubsetData(df3, c("CaseID","Birth_Year", "Diag","SrceIDs")) %>%
   ...
   bindCache(input$t0, input$tn, input$icd10, input$src)
  ...
  output$srceplot <- renderPlotly({
   ...
   validate(need(ncol(upset()) > 2,
                 "Sorry, there is no data available for plotting.
             \nPlease, select more 'Data source' options."))
