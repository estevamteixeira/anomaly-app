...
src_lbl <- as.vector(sort(unique(ano$SrceIDs)))

if (length(src_lbl) %in% 7) {
 names(src_lbl) <- c("FADB", "NSAPD", "CARDIO", "CIHI", "MSI", "NeoNatal", "Others")
} else if (length(src_lbl) %in% 6) {
 names(src_lbl) <- c("FADB", "NSAPD", "CIHI", "MSI", "NeoNatal", "Others")
}
